...
export class AnalyticsService {
    //private static registeredEvents =  Array<{key: string, fields:Array<{name: string,  key: string}>}>()
    ...
    registerEvents(){        
        for (let evt of AnalyticsService.registeredEvents){            
            ...
            window.addEventListener(evt.key, (event) => this.logEvent(event, evt), isGlobal);
        } 
    }
        ...
        if (event.type == 'load') {
            this.http
                ...
                .subscribe((geodata: any) => {
                    ...
                    //this.http.post("/api/analytics", metric).take(1).subscribe();
                }, (error) =>{
                   // this.http.post("/api/analytics", metric).take(1).subscribe();
                });
        }
